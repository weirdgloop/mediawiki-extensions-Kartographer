...
L.Draw.Feature = L.Handler.extend({
	...
	initialize: function (map, options) {
		...
		var shapeOptions = {
			...
			plane: this._map._plane
		}
		// Merge default shapeOptions options with custom shapeOptions
		if (options && options.shapeOptions) {
			options.shapeOptions = L.Util.extend({}, this.options.shapeOptions, options.shapeOptions, shapeOptions);
		}
...
L.Draw.Marker = L.Draw.Feature.extend({
	...
	options: {
		...
		icon: new L.icon({
			icon: "pin_blue.svg",
			iconUrl: (mw.config.get( 'wgKartographerDataConfig' ).iconURL + "pin_blue.svg"),
			iconSize: [26, 42],
			iconAnchor: [13, 42],
			popupAnchor: [0, -42]	
		}),
